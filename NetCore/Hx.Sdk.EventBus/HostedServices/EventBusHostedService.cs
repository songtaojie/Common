...
namespace Hx.Sdk.EventBus
{
    ...
    internal sealed class EventBusHostedService : BackgroundService
    {
        ...
        /// 事件处理程序集合
        /// </summary>
        private readonly ConcurrentDictionary<EventHandlerWrapper, EventHandlerWrapper> _eventHandlers = new ConcurrentDictionary<EventHandlerWrapper, EventHandlerWrapper>();
        ...
        /// <param name="enabledLog">是否启用日志记录</param>
                    ...
                    try
                    {
                        ...
                        if (Executor == default)
                        {
                            ...
                            var policy = Policy.Handle<SocketException>()
                                ...
                                .WaitAndRetry(eventSubscribeAttribute?.NumRetries ?? 0, retryAttempt =>
                                {
                                    if (retryTimeout.Length >= retryAttempt)
                                        return retryTimeout[retryAttempt - 1];
                                    return retryTimeout.Last();
                                },
                                (ex, time) =>
                                {
                                    // 输出重试日志
