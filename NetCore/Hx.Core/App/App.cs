...
namespace Hx.Core
{
    ...
    public static partial class App
    {
        ...
        {
            return (InternalApp.Configuration ?? GetService<IConfiguration>()).GetSection(jsonKey).Get<TOptions>();
        }
        ...
        public static string GetConfig(params string[] sections)
        {
            try
            {
                ...
                {
                    return (InternalApp.Configuration ?? GetService<IConfiguration>()) [sections[0]];
                }
                else
                {
                    string section = string.Join(':', sections);
                    return (InternalApp.Configuration ?? GetService<IConfiguration>())[section];
                }
            }
            catch
            {
